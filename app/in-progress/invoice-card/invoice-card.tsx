    ...
    return (
        <div className={cn("h-[500px] flex justify-center items-center w-full", className)}>
            ...
            <motion.div
                ...
                className="border border-[#E8E8E8] dark:border-neutral-700 px-5 py-6 rounded-[20px] bg-[#fafafa] dark:bg-neutral-800 h-auto flex flex-col min-w-[350px]  overflow-hidden">
                ...
                <div className="mt-8 space-y-4">
                        ...
                        {items.map((item, index) => (
                            <motion.div
                                ...
                                <div className="flex-1">
                                    <p className="text-sm font-medium text-black dark:text-white mb-1">{item.name}</p>
                                    <p className="text-xs text-neutral-500 dark:text-neutral-400">{item.description}</p>
                                </div>
                                <p className="text-sm font-medium text-black dark:text-white">${item.price.toLocaleString()}</p>
                            </motion.div>
                        ))}
                    </div>
                    ...
                    <motion.div
                        ...
                        className="border-t border-neutral-200 dark:border-neutral-700 my-4"></motion.div>
                    ...
                    <div className="space-y-2">
                        <motion.div
                            ...
                            <p className="text-sm font-medium text-black dark:text-white">${subtotal.toLocaleString()}</p>
                        </motion.div>
                        {taxRate > 0 && (
                            <motion.div
                                initial={{ x: -150, opacity: 0 }}
                                ...
                                <p className="text-sm font-medium text-black dark:text-white">${tax.toLocaleString()}</p>
                            </motion.div>
                        )}
                        <motion.div
                            initial={{ x: -150, opacity: 0 }}
                            ...
                            <p className="text-base font-semibold text-black dark:text-white">Total</p>
                            <p className="text-base font-semibold text-black dark:text-white">${total.toLocaleString()}</p>
                        </motion.div>
                    </div>
                </div>
            </motion.div>
