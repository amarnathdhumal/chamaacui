...
// const GridItem = ({ src, isLeftSide, scrollContainer }: GridItemProps) => {
//   const ref = useRef<HTMLDivElement>(null)

//   // Use the scroll container directly (no ref issues since it's passed as element)
...
//   const rotateZEnter = useTransform(scrollYProgress, [0, 0.5], [isLeftSide ? 5 : -5, 0])
//   const xPercentEnter = useTransform(scrollYProgress, [0, 0.5], [isLeftSide ? -40 : 40, 0])
//   const skewXEnter = useTransform(scrollYProgress, [0, 0.5], [isLeftSide ? -20 : 20, 0])
...
//   const rotateZExit = useTransform(scrollYProgress, [0.5, 1], [0, isLeftSide ? -1 : 1])
//   const xPercentExit = useTransform(scrollYProgress, [0.5, 1], [0, isLeftSide ? -20 : 20])
//   const yPercentExit = useTransform(scrollYProgress, [0.5, 1], [0, -80])

//   const skewXExit = useTransform(scrollYProgress, [0.5, 1], [0, isLeftSide ? 20 : -20])

//   // Combine enter and exit transforms
//   const z = useTransform(() => scrollYProgress.get() < 0.5 ? zEnter.get() : zExit.get())
//   const rotateX = useTransform(() => scrollYProgress.get() < 0.5 ? rotateXEnter.get() : rotateXExit.get())
//   const rotateZ = useTransform(() => scrollYProgress.get() < 0.5 ? rotateZEnter.get() : rotateZExit.get())
//   const xPercent = useTransform(() => scrollYProgress.get() < 0.5 ? xPercentEnter.get() : xPercentExit.get())
//   const skewX = useTransform(() => scrollYProgress.get() < 0.5 ? skewXEnter.get() : skewXExit.get())
//   const yPercent = useTransform(() => scrollYProgress.get() < 0.5 ? yPercentEnter.get() : yPercentExit.get())

//   // Filter animations
//   const blurEnter = useTransform(scrollYProgress, [0, 0.5], [7, 0])
//   const blurExit = useTransform(scrollYProgress, [0.5, 1], [0, 4])
//   const blur = useTransform(() => scrollYProgress.get() < 0.5 ? blurEnter.get() : blurExit.get())
...
//   const brightness = useTransform(() => scrollYProgress.get() < 0.5 ? brightnessEnter.get() : brightnessExit.get())
...
//   const contrast = useTransform(() => scrollYProgress.get() < 0.5 ? contrastEnter.get() : contrastExit.get())

//   // Inner image scale
//   const scaleYEnter = useTransform(scrollYProgress, [0, 0.5], [1.8, 1])
//   const scaleYExit = useTransform(scrollYProgress, [0.5, 1], [1, 1.8])
//   const scaleY = useTransform(() => scrollYProgress.get() < 0.5 ? scaleYEnter.get() : scaleYExit.get())
...
// const TextElement = ({ gridRef, scrollContainer }: TextElementProps) => {
//   // Use the scroll container directly
//   const { scrollYProgress } = useScroll({
//     target: gridRef,
//     container: scrollContainer,
//     offset: ["start end", "end start"],
...
//     >
//       <p className="text-4xl uppercase tracking-[0.2em] text-white opacity-60">
//         Gallery Collection
//       </p>
//     </motion.div>
//   )
...
// interface StaggeredScrollContainerDemoProps {
//   itemCount?: number;
// }
...
//  */
// const StaggeredScrollContainerDemo = ({ itemCount = 30 }: StaggeredScrollContainerDemoProps = {}) => {
//   const scrollContainerRef = useRef<HTMLDivElement>(null)
//   const gridRef = useRef<HTMLDivElement>(null)
//   const [scrollContainer, setScrollContainer] = useState<HTMLDivElement | null>(null)
//   const items = Array.from({ length: itemCount }, (_, i) => `${(i % 4) + 1}.jpg`)

//   // Set the scroll container once it's mounted
//   useEffect(() => {
...
//           >
//             {scrollContainer && items.map((src, index) => (
//               <GridItem
//                 key={index}
//                 src={src}
...
//           {scrollContainer && (
//             <TextElement gridRef={gridRef} scrollContainer={scrollContainer} />
//           )}
//         </section>
//       </main>
//     </div>
